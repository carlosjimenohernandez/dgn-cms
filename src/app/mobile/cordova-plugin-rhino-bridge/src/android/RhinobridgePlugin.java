...
public class RhinobridgePlugin extends CordovaPlugin {
    ...
    private void initializeRhinoScope() {
        ...
        ScriptableObject.putProperty(scope, "$rhino", Context.javaToJS(this.rhino, scope));
    ...
    private void evaluateScript(String script, CallbackContext callbackContext) {
        boolean hasPreviousScope = this.scope != null;
        try {
            this.rhino = Context.enter(); // Inicia el entorno Rhino
            this.rhino.setOptimizationLevel(-1); // Evita optimizaciones para mayor compatibilidad
            if(!hasPreviousScope) {
                this.scope = rhino.initStandardObjects(); // Crea el contexto global
            }
            ...
            if(!hasPreviousScope) {
                this.initializeRhinoScope();
            }
            Object result = this.rhino.evaluateString(this.scope, script, "JavaScript", 1, null); // Ejecuta el script
